...
export class Bot extends Component {
    ...
    promotingPawn() {
        if (BoardState.nextBotMove[1][0] == 0) {
            ...
            if (Math.abs(BoardState.nextBotMove[1][1] - BoardState.nextBotMove[0][1]) == 1) {
                BoardState.justCaptured = true;
            }
        }
        else if (BoardState.nextBotMove[1][0] == 7) {
            let pieceBag: number[] = [-8, -5, -3, -2];
            BoardState.piecePromoted = pieceBag[Math.floor(Math.random() * 4)];
            if (BoardState.piecePromoted == -8) {
    ...
    update(deltaTime: number) {
        ...
        if (BoardState.turn == -Math.sign(this.pieceName) && BoardState.nextBotMove[0][0] > -1) {
            ...
            && BoardState.convertCoordinates(this.piecePosition)[1] == BoardState.nextBotMove[0][1]) {
                if (BoardState.board[BoardState.nextBotMove[1][0]][BoardState.nextBotMove[1][1]] != 0) {
                    BoardState.justCaptured = true;
                }
                BoardState.setBoard(0, this.piecePosition);
                BoardState.setBoard(this.pieceName, BoardState.convertUIPosition(BoardState.nextBotMove[1]));
                ...
                BoardState.previousPosition.x = BoardState.convertUIPosition(BoardState.nextBotMove[0]).x;
                BoardState.previousPosition.y = BoardState.convertUIPosition(BoardState.nextBotMove[0]).y;
                BoardState.currentMove.x = BoardState.convertUIPosition(BoardState.nextBotMove[1]).x;
                BoardState.currentMove.y = BoardState.convertUIPosition(BoardState.nextBotMove[1]).y;
